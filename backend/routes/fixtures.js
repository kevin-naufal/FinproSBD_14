...
router.post('/', async (req, res) => {
  const { home_club_id, away_club_id, home_score, away_score, league_id, matchweek_id, match_date, referee_id } = req.body;
  try {
    const result = await pool.query(
      'INSERT INTO fixtures (home_club_id, away_club_id, home_score, away_score, league_id, matchweek_id, match_date, referee_id) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *',
      [home_club_id, away_club_id, home_score, away_score, league_id, matchweek_id, match_date, referee_id]
    );
...
router.put('/:id', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'UPDATE fixtures SET home_club_id = $1, away_club_id = $2, home_score = $3, away_score = $4, league_id = $5, matchweek_id = $6, match_date = $7, referee_id = $8 WHERE id = $9 RETURNING *',
      [home_club_id, away_club_id, home_score, away_score, league_id, matchweek_id, match_date, referee_id, id]
    );
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Fixture not found' });
    }
