    ...
    const fetchFixtures = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/api/fixtures/matchweek/${selectedMatchweek}`
        );
    ...
    <PageContent>
      <Container maxWidth="md">
        <Box className="mb-8 p-6 border rounded-lg shadow-lg bg-white w-full max-w-4xl border-2 border-black bg-[#609966]">
          <label htmlFor="matchweek" className="block mb-2">
            Select Matchweek:{" "}
          </label>
          <select
            id="matchweek"
        ...
        {Object.keys(groupedFixtures).map((leagueName, index) => (
          <Box
            key={index}
            className="mb-8 p-6 border rounded-lg shadow-lg bg-white w-full max-w-4xl border-2 border-black bg-[#9DC08B]"
          >
            <Typography
              variant="h4"
              component="h2"
            ...
            >
              <Button
                ...
                onClick={() =>
                  handleLeagueClick(groupedFixtures[leagueName][0].league_id)
                }
                ...
                <TableBody>
                  {groupedFixtures[leagueName].map((fixture, fixtureIndex) => (
                    <TableRow
                      key={fixtureIndex}
                      className="bg-[#EDF1D6]"
                      onClick={() => handleCardClick(fixture.id)}
                      style={{ cursor: "pointer" }}
                    >
                      <TableCell className="club-name-hover">
                        <Link
                          to={`/club/${fixture.home_club_id}`}
