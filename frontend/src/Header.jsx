...
const Header = () => {
  ...
  const [searchResults, setSearchResults] = useState({ players: [], clubs: [] });
  ...
  useEffect(() => {
    ...
    const fetchLoginStatus = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/login/latest-login-session');
    ...
    <header style={{ position: 'fixed', top: 0, width: '100%', background: '#333', color: '#fff', padding: '30px 20px', textAlign: 'center', zIndex: 999, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      ...
      <form onSubmit={handleSearch} style={{ position: 'relative' }}>
        ...
        {searchQuery.length >= 3 && (
          <div className="search-results" style={{ position: 'absolute', top: '100%', left: 0, zIndex: 999, backgroundColor: '#fff', border: '1px solid #ccc', borderRadius: '5px', width: '200px', marginTop: '20px' }}>
            {searchResults.players.length > 0 && (
              <div>
                <h3 style={{ color: '#000', fontWeight: 'bold', marginBottom: '10px' }}>Players</h3>
                <ul>
            ...
            {searchResults.clubs.length > 0 && (
              <div>
                <h3 style={{ color: '#000', fontWeight: 'bold', marginBottom: '10px', marginTop: '10px' }}>Clubs</h3>
                <ul>
      ...
      {isLoggedIn ? (
        <button onClick={handleLogoutClick} style={{ background: '#555', color: '#fff', border: 'none', padding: '10px 20px', cursor: 'pointer' }}>
          Logout
        </button>
      ) : (
        <button onClick={handleLoginClick} style={{ background: '#555', color: '#fff', border: 'none', padding: '10px 20px', cursor: 'pointer' }}>
          Log In
        </button>
      )}
    </header>
